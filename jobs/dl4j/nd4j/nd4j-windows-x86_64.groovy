        ...
        for (lib in nd4jlibs) {
            ...
            configFileProvider([configFile(fileId: settings_xml, variable: 'MAVEN_SETTINGS')]) {
                ...
                bat (
                        'vcvars64.bat' +
                                '&&' +
                                'bash -c "export PATH=$PATH:/c/msys64/mingw64/bin && mvn -B -s %WORKSPACE_BASH%/settings.xml clean deploy -Dlocal.software.repository=%PROFILE_TYPE% -DstagingRepositoryId=%STAGE_REPO_ID% -DperformRelease=%GpgVAR% -Dmaven.test.skip=%SKIP_TEST% -Denv.LIBND4J_HOME=%LIBND4J_HOME%'
                )
