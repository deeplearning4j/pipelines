...
def get_libnd4j_artifacts_snapshot_ball(some_version, some_platform, profile_type) {
    switch (profile_type) {
        case "nexus":
            if (isUnix()) {
                sh("mvn -B dependency:get -DrepositoryUrl=http://jenkins-master.eastus.cloudapp.azure.com:8088/nexus/content/repositories/snapshots " +
                ...
                bat("mvn -B dependency:get -DrepositoryUrl=http://jenkins-master.eastus.cloudapp.azure.com:8088/nexus/content/repositories/snapshots " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
        ...
        case "sonatype":
            ...
            if (isUnix()) {
                sh("mvn -B dependency:get -DrepositoryUrl=https://oss.sonatype.org/content/repositories/snapshots " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
                        "-Ddest=${LIBPROJECT}-${some_version}-${some_platform}.zip ")
            } else {
                bat("mvn -B dependency:get -DrepositoryUrl=https://oss.sonatype.org/content/repositories/snapshots " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
                        "-Ddest=${LIBPROJECT}-${some_version}-${some_platform}.zip ")
        ...
        case "bintray":
            if (isUnix()) {
                sh("mvn -B dependency:get -DrepositoryUrl=https://oss.jfrog.org/artifactory/oss-snapshot-local " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
                        "-Ddest=${LIBPROJECT}-${some_version}-${some_platform}.zip ")
            } else {
                bat("mvn -B dependency:get -DrepositoryUrl=https://oss.jfrog.org/artifactory/oss-snapshot-local " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
                        "-Ddest=${LIBPROJECT}-${some_version}-${some_platform}.zip ")
        ...
        case "jfrog":
            if (isUnix()) {
                sh("mvn -B dependency:get -DrepositoryUrl=http://jenkins-master.eastus.cloudapp.azure.com:8081/artifactory/libs-snapshot-local " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
                        "-Ddest=${LIBPROJECT}-${some_version}-${some_platform}.zip ")
            } else {
                bat("mvn -B dependency:get -DrepositoryUrl=http://jenkins-master.eastus.cloudapp.azure.com:8081/artifactory/libs-snapshot-local " +
                        "-DgroupId=org.nd4j -DartifactId=${LIBPROJECT} -Dversion=${VERSION} -Dpackaging=zip " +
                        "-Dtransitive=false " +
                        "-Dclassifier=${some_platform} " +
                        "-Ddest=${LIBPROJECT}-${some_version}-${some_platform}.zip ")
