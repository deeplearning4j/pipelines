    ...
    ws(WORKSPACE + "_" + PLATFORM_NAME) {
        properties([
                ...
                [$class: "ParametersDefinitionProperty", parameterDefinitions:
                        [
                                [$class: "StringParameterDefinition", name: "VERSION", defaultValue: "1.0.0-SNAPSHOT", description: "Deeplearning component release version"],
...
//                            [$class: "ChoiceParameterDefinition", name: "CUDA_VERSION", choices: "8.0\n9.0", description: "Cuda version definition"],
...
//                            [$class: "StringParameterDefinition", name: "PDIR", defaultValue: "jobs/dl4j", description: "Path to groovy scripts"],
                            ...
                            [$class: "BooleanParameterDefinition", name: "CREATE_RPM", defaultValue: false, description: "Select to create rpm for release"]
                ]]
        ])
        ...
        stage("${SKIL_PROJECT}") {
            load "${PDIR}/${SKIL_PROJECT}/${SKIL_PROJECT}-${PLATFORM_NAME}.groovy"
        }
        ...
        stage('RELEASE') {
            ...
            if (isSnapshot) {
                echo "End of building and publishing of the ${SKIL_PROJECT}-${VERSION}"
            } else {
                // timeout(time:1, unit:'HOURS') {
                timeout(20) {
                    input message: "Approve release of version ${SKIL_PROJECT}-${VERSION} ?"
                }

                functions.tag("${SKIL_PROJECT}")
            }
...
ansiColor('xterm') {
    echo "\033[42m MARK: end of skil/build.groovy \033[0m"
}
