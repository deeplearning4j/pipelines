...
//                            [$class: "StringParameterDefinition", name: "PDIR", defaultValue: "jobs/dl4j", description: "Path to groovy scripts"],
        ...
        // dockerParams = "-v ${WORKSPACE}:${WORKSPACE}:rw -v ${WORKSPACE}/.m2:/home/jenkins/.m2:rw --device=/dev/nvidiactl --device=/dev/nvidia-uvm --device=/dev/nvidia0 --volume=nvidia_driver_375.26:/usr/local/nvidia:ro"
        ...
        stage("${DATAVEC_PROJECT}") {
            load "${PDIR}/${DATAVEC_PROJECT}/${DATAVEC_PROJECT}-${PLATFORM_NAME}.groovy"
        }
        ...
        stage('RELEASE') {
            ...
            if (isSnapshot) {
                echo "End of building and publishing of the ${DATAVEC_PROJECT}-${VERSION}"
            } else {
